package com.akjava.gwt.clothhair.client;

public class GWTThreeClothHairStorageKeys {
public static final String temp_sphere="tmp_sphere";

public static final String KEY_SPHERES="spheres";

public static final String temp_hairset="tmp_hairset";

public static final String THREEJS_RENDERER_ANTIALIAS="threejs_renderer_antialias";
public static final String THREEJS_CLEAR_COLOR="threejs_clear_color";
public static final String KEY_GROUND_COLOR="ground_color";

public static final String KEY_LIGHTS="threejs_lights";

public static final String KEY_AMMO_WORLD_SCALE="ammo_world_scale";
public static final String KEY_AMMO_PARTICLE_BODY="ammo_particle_body";
public static final String KEY_AMMO_PARTICLE_CONSTRAINT="ammo_particle_constraint";
}
