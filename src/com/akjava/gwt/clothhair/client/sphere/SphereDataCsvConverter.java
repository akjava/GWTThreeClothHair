...
public class SphereDataCsvConverter extends Converter<SphereData,String>{

	public SphereDataCsvConverter() {
		
	}

	@Override
	protected String doForward(SphereData value) {
		return Joiner.on(",").join(Doubles.asList(new double[]{value.getX(),value.getY(),value.getZ(),value.getWidth()
		,value.isEnabled()?1.0:0,value.getBoneIndex(),value.getChannel()		,value.isCopyHorizontal()?1.0:0
				,value.getType(),
				value.getRotation().getX(),value.getRotation().getY(),value.getRotation().getZ(),value.getRotation().getW()
		}));
	}
